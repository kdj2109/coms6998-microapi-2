...
@app.route('/' + microservice_name + '/<id>/<grade>', methods=['PUT'])
def put(id, grade):
    json_dict = request.get_json()
    ...
    response = app_dao.update(json_dict, grade, id)
...
@app.route('/' + microservice_name + '/<id>/<grade>', methods=['DELETE'])
def delete(id, grade):
    response = app_dao.delete(grade, id)

    return response


def valid_json(json_dict):
    if all(key in json_dict for key in ('first_name', 'last_name', 'grade')):
        if 0 <= int(json_dict['grade']) <= 12:
            return True
